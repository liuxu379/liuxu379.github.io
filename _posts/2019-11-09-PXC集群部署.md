 ...
 **Percona-XtraDB-Cluster**是 MySQL 集群的一个主主复制的解决方案，暂时也只了解了这一个，搭配 HaProxy 负载均衡，自己用起来感觉效果不错。简单介绍一下其特点：
 ...
 1. 强一致性，所有服务器提交的事务全部执行完，才算执行成功；
 2. 只支持 InnoDB 引擎；
 3. 所有服务器的配置都要相同，因为同步速度取决于配置最低的服务器；
 4. 多主复制，可以在任一节点写入，可以很方便地做负载均衡；
 5. 数据库过多的话会影响同步速度；

介绍完 PXC，再简单说一下 **Replication** 的特点：
1. 弱一致性，异步复制，无法保证数据一致性；(所以说负载均衡对于这种模式来说是否是灾难性的？)
2. 同步复制快；
3. 适合存储低价值的数据；(所以说在某些论坛里发帖，发完后莫名其妙找不到就是这个原因？)
